function mCADload(car)

% Load trailer (carretinha) 3D CAD model 
car.pCAD.obj{1} = loadawobj('semitrailerNEROcolored.obj');
car.pCAD.mtl{1} = loadawmtl('semitrailerNEROcolored.mtl');


% Load trailer (carretinha fechada) 3D CAD model 
car.pCAD.obj{2} = loadawobj('semitrailerNEROcolored_Closed.obj');
car.pCAD.mtl{2} = loadawmtl('semitrailerNEROcolored_Closed.mtl');

% Load reboque 3D CAD model 
car.pCAD.obj{3} = loadawobj('reboquePioneer3Dx.obj');
car.pCAD.mtl{3} = loadawmtl('reboquePioneer3Dx.mtl');


% Flags
car.pCAD.flagLoaded  = 1;  % 3D image loaded
car.pCAD.flagCreated = 0;  % 3D image created using Matlab Patch function
car.pCAD.flagLines   = 1;  % Illustrate lines between and surface

car.pCAD.flagpPlot = 1;


for i = 1:3
    
    % Ajustando Dimensões:
    vExt = minmax(car.pCAD.obj{i}.v);
    mExt = mean(vExt,2);

        
    for ii = 1:3
        if i~= 3
            car.pCAD.obj{i}.v(ii,:) = (car.pCAD.obj{i}.v(ii,:) - mExt(ii))/900;
        else
            car.pCAD.obj{i}.v(ii,:) = (car.pCAD.obj{i}.v(ii,:) - mExt(ii))/950;
        end
    end


    % Ajustando a orientação e posição iniciais:
    car.pCAD.obj{i}.v = [1 0 0; 0 0 -1; 0 1 0]*car.pCAD.obj{i}.v;   % Rx,theta = [1 0 0;
                                                                    %             0 cos(theta) -sen(theta);
                                                                    %             0 sen(theta) cos(theta)];

    if i ~= 3
        car.pCAD.obj{i}.v(1,:) = car.pCAD.obj{i}.v(1,:) + (0.1125+0.0125-0.3791)*ones(1,length(car.pCAD.obj{i}.v(1,:)));
        %car.pCAD.obj{i}.v(1,:) = car.pCAD.obj{i}.v(1,:) + (0.1125+0.0125-0.018)*ones(1,length(car.pCAD.obj{i}.v(1,:)));
        car.pCAD.obj{i}.v(3,:) = car.pCAD.obj{i}.v(3,:) - min(car.pCAD.obj{i}.v(3,:))*ones(1,length(car.pCAD.obj{i}.v(3,:)));
    else
        car.pCAD.obj{i}.v(1,:) = car.pCAD.obj{i}.v(1,:) + (0.021)*ones(1,length(car.pCAD.obj{i}.v(1,:)));
        car.pCAD.obj{i}.v(2,:) = car.pCAD.obj{i}.v(2,:) + (0)*ones(1,length(car.pCAD.obj{i}.v(2,:)));
        car.pCAD.obj{i}.v(3,:) = car.pCAD.obj{i}.v(3,:) - (min(car.pCAD.obj{3}.v(3,:)) - 0.1366 + 0.044)*ones(1,length(car.pCAD.obj{i}.v(3,:)));

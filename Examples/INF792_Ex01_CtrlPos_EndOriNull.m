...
while toc(t) < tsim
    ...
    if toc(tc) > tap
        ...
        % Control:
        if P.pPos.rho > 0.00003 % Limite para erro de posição!
            ...
            if abs(P.pPos.alpha) >= alpha_min
                P.pSC.Ud(2) = k_alpha*tanh(P.pPos.alpha) + k_theta*P.pPos.theta*(tanh(P.pPos.theta)/P.pPos.alpha) + umax*(tanh(P.pPos.rho)/P.pPos.rho)*cos(P.pPos.alpha)*sin(P.pPos.alpha) + P.pPos.theta*umax*(tanh(P.pPos.rho)/P.pPos.rho)*(sin(P.pPos.alpha)/sin(P.pPos.alpha))*cos(P.pPos.alpha);
            else
                P.pSC.Ud(2) = k_alpha*alpha_min + k_theta*P.pPos.theta*(tanh(P.pPos.theta)/alpha_min) + umax*P.pPos.theta*(tanh(P.pPos.rho)/P.pPos.rho);
            end
    ...
    if toc(tp) > tap
        ...
        P.mCADplot;
        hold on
        h(1) = plot(Rastro.Xd(:,1),Rastro.Xd(:,2),'xk','MarkerSize',8.5,'LineWidth',1.5);
        h(2) = plot(Rastro.X(:,1),Rastro.X(:,2),'g','LineWidth',1.5);
        axis([-3 3 -3 3])
        grid on
        hold off
        drawnow
